...
var Client = /** @class */ (function () {
    function Client(config) {
        ...
        var clonedConfig = tslib_1.__assign(tslib_1.__assign({}, this._config), { basicAuthCredentials: this._config.basicAuthCredentials || {
                username: this._config.basicAuthUserName || '',
                password: this._config.basicAuthPassword || ''
            } });
        this._userAgent = (0, core_3.updateUserAgent)('PagarmeApiSDK - TypeScript 6.8.11');
        this._requestBuilderFactory = createRequestHandlerFactory(function (server) { return getBaseUri(server, _this._config); }, (0, authProvider_1.createAuthProviderFromConfig)(clonedConfig), new clientAdapter_1.HttpClient(core_4.AbortError, {
...
function createRequestHandlerFactory(baseUrlProvider, authProvider, httpClient, addons, retryConfig) {
    var requestBuilderFactory = (0, core_4.createRequestBuilderFactory)(createHttpClientAdapter(httpClient), baseUrlProvider, core_1.ApiError, authProvider, retryConfig);
...
function withErrorHandlers(rb) {
    rb.throwOn(400, customError_1.CustomError, 'Invalid request');
    rb.throwOn(401, customError_1.CustomError, 'Invalid API key');
    rb.throwOn(404, customError_1.CustomError, 'An informed resource was not found');
    rb.throwOn(412, customError_1.CustomError, 'Business validation error');
    rb.throwOn(422, customError_1.CustomError, 'Contract validation error');
    rb.throwOn(500, customError_1.CustomError, 'Internal server error');
}
function withUserAgent(userAgent) {
    return function (rb) {
        rb.interceptRequest(function (request) {
            ...
            (0, core_2.setHeader)(headers, 'user-agent', userAgent);
            return tslib_1.__assign(tslib_1.__assign({}, request), { headers: headers });
        });
    };
}
function withServiceRefererName(_a) {
    var serviceRefererName = _a.serviceRefererName;
    return function (rb) {
        rb.interceptRequest(function (request) {
            var _a;
            var headers = (_a = request.headers) !== null && _a !== void 0 ? _a : {};
            (0, core_2.setHeader)(headers, 'ServiceRefererName', serviceRefererName);
            return tslib_1.__assign(tslib_1.__assign({}, request), { headers: headers });
        });
    };
}
